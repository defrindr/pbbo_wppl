...
class KuesionerKepuasanController extends Controller {
    ...
    public function actionRequestSoal($id, $unique_id){
        ...
        $pelatihan_soal = MasterKuesionerKepuasan::findOne(['id' => $id]);
        $jawaban = PelatihanKuesionerKepuasan::findOne(['peserta_id' => $peserta->id, "jenis_id" => $pelatihan_soal_jenis->id, 'soal' => $pelatihan_soal->id]);        
    ...
    public function actionIndex($unique_id = null){
        ...
        $user = \Yii::$app->user->identity;
        $model_jenis_soal = $model->getPelatihanSoalJenis()->where(['jenis_id' => Constant::SOAL_JENIS_KUESIONER_KEPUASAN])->one();
        $soals = MasterKuesionerKepuasan::find()->all();
