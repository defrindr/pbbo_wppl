...
const requestSoal = (unique) => {
    ...
    fetch("'. \yii\helpers\Url::to(['/kuesioner-monev/request-soal/']).'/"+unique+"/"+unique_id).then(response => response.text()).then(response => {
...
const updateJawaban = () => {
    ...
    $.ajax({
        url: "'. \yii\helpers\Url::to(['kuesioner-monev/post-answer']). '",
        method: "POST",
        data: data
    });
}
...
const selesai = () => {
    updateJawaban();
    $.ajax({
        url: "'. \yii\helpers\Url::to(['kuesioner-monev/finish']). '",
        method: "POST",
        data: $("#form-soal").serialize()
    });
}
